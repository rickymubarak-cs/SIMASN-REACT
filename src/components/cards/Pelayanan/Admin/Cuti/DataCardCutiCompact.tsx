// src/components/cards/DataCardCutiCompact.tsx
import React from 'react';
import { Calendar, Eye, Edit, Ban, CheckCircle, Upload } from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';

interface DataCardCutiCompactProps {
    data: any;
    index: number;
    onDetail: () => void;
    onPerbaiki?: () => void;
...
export const DataCardCutiCompact: React.FC<DataCardCutiCompactProps> = ({
    data,
    index,
    onDetail,
    onPerbaiki,
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 hover:shadow-lg hover:border-cyan-200 transition-all group">
            <div className="p-3">
                ...
                <div className="text-[9px] text-cyan-600 bg-cyan-50 px-1.5 py-0.5 rounded-full inline-block mb-2">
                    {jenisCuti}
                </div>
                ...
                <div className="flex gap-1 mt-2 pt-2 border-t border-slate-100">
                    <button
                        onClick={onDetail}
                        className="flex-1 py-1.5 rounded-lg text-[9px] font-bold bg-slate-50 text-slate-600 hover:bg-cyan-50 hover:text-cyan-600 transition-all"
                    >
                    ...
                    {status === "diterima" && (
                        <button
                            onClick={onUpload}
                            className="flex-1 py-1.5 rounded-lg text-[9px] font-bold bg-cyan-50 text-cyan-600 hover:bg-cyan-600 hover:text-white transition-all"
                        >
