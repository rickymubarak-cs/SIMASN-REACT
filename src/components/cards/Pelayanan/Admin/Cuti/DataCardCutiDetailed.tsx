// src/components/cards/DataCardCutiDetailed.tsx
import React from 'react';
import { Calendar, Clock, Eye, Edit, Ban, CheckCircle, Upload, Calendar as CalendarIcon, Building } from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';
import { formatDateTimeId } from '../../../../../utils/formatters';

interface DataCardCutiDetailedProps {
    data: any;
    index: number;
    onDetail: () => void;
    onPerbaiki?: () => void;
...
export const DataCardCutiDetailed: React.FC<DataCardCutiDetailedProps> = ({
    data,
    index,
    onDetail,
    onPerbaiki,
    ...
    const getJenisCutiColor = () => {
        if (jenisCuti === "TAHUNAN") return "text-cyan-600";
        if (jenisCuti === "SAKIT") return "text-blue-600";
        if (jenisCuti === "BESAR") return "text-purple-600";
        return "text-cyan-500";
    };

    return (
        <div className="bg-white rounded-2xl border border-slate-200 hover:shadow-xl hover:border-cyan-200 transition-all group">
            <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black text-slate-800 text-base uppercase group-hover:text-cyan-600 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                        <CalendarIcon size={14} className={getJenisCutiColor()} />
                        <span>Jenis: {jenisCuti}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                        <span className="text-cyan-500 font-bold">#{index}</span>
                    </div>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-100">
                    <button
                        onClick={onDetail}
                        className="flex-1 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-700 hover:bg-cyan-100 hover:text-cyan-700 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    {status === "diterima" && (
                        <button
                            onClick={onUpload}
                            className="flex-1 py-2 rounded-xl text-xs font-bold bg-cyan-100 text-cyan-700 hover:bg-cyan-600 hover:text-white transition-all flex items-center justify-center gap-2"
                        >
