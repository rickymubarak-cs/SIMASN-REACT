// src/components/cards/Pelayanan/Admin/Diklat/DataCardDiklat.tsx

import React, { useState } from 'react';
import { BookOpen, Clock, Eye, Edit, Ban, CheckCircle, Upload, FileCheck, Building, Loader } from 'lucide-react';
...
export const DataCardDiklat: React.FC<DataCardDiklatProps> = ({
    ...
    const handleTerimaClick = async () => {
        ...
        try {
            await onTerima?.(data?.layanan_diklat_id, false);
        } finally {
            ...
            <div className="bg-slate-50/50 p-4 space-y-2 border-t border-slate-100">
                <div className="flex gap-1.5 flex-wrap">
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border border-sky-200 text-sky-600 hover:bg-sky-600 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    {(status === "diterima" || status === "selesai") && (
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border transition-all hover:text-white ${status === "selesai"
                                ? 'border-sky-200 text-sky-600 hover:bg-sky-600'
