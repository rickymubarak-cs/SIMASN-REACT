// src/components/cards/DataCardGajiDetailed.tsx
import React from 'react';
import { DollarSign, Clock, Eye, Edit, Ban, CheckCircle, Upload, Calendar, Building, TrendingUp } from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';
import { formatDateTimeId } from '../../../../../utils/formatters';

interface DataCardGajiDetailedProps {
    data: any;
    index: number;
    onDetail: () => void;
    onPerbaiki?: () => void;
...
export const DataCardGajiDetailed: React.FC<DataCardGajiDetailedProps> = ({
    data,
    index,
    onDetail,
    onPerbaiki,
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 hover:shadow-xl hover:border-teal-200 transition-all group">
            <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black text-slate-800 text-base uppercase group-hover:text-teal-600 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                        <span className="text-teal-500 font-bold">#{index}</span>
                    </div>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-100">
                    <button
                        onClick={onDetail}
                        className="flex-1 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-700 hover:bg-teal-100 hover:text-teal-700 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    {status === "diterima" && (
                        <button
                            onClick={onUpload}
                            className="flex-1 py-2 rounded-xl text-xs font-bold bg-teal-100 text-teal-700 hover:bg-teal-600 hover:text-white transition-all flex items-center justify-center gap-2"
                        >
