// src/components/cards/DataCardKompetensiCompact.tsx
import React from 'react';
import { Award, Eye, Edit, Ban, CheckCircle, Upload } from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';

interface DataCardKompetensiCompactProps {
    data: any;
    index: number;
    onDetail: () => void;
    onPerbaiki?: () => void;
...
export const DataCardKompetensiCompact: React.FC<DataCardKompetensiCompactProps> = ({
    data,
    index,
    onDetail,
    onPerbaiki,
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 hover:shadow-lg hover:border-orange-200 transition-all group">
            <div className="p-3">
                ...
                <div className="text-[9px] text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded-full inline-block mb-1">
                    {jenisKompetensi}
                </div>

                <div className="text-[8px] text-slate-400 mb-2 line-clamp-1">
                    {data.riw_kom_nama?.slice(0, 30) || "-"}
                </div>

                <div className="flex gap-1 mt-2 pt-2 border-t border-slate-100">
                    <button
                        onClick={onDetail}
                        className="flex-1 py-1.5 rounded-lg text-[9px] font-bold bg-slate-50 text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition-all"
                    >
