// src/components/cards/DataCardPangkat.tsx
import React from 'react';
import { TrendingUp, Clock, Eye, Edit, Ban, CheckCircle, Upload, FileCheck, Building, Award } from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';
import { formatDateTimeId } from '../../../../../utils/formatters';
import { pangkatFileConfig } from '../../../../../service/pangkatService';

interface DataCardPangkatProps {
    data: any;
    index: number;
    onDetail: () => void;
    onPerbaiki?: () => void;
...
export const DataCardPangkat: React.FC<DataCardPangkatProps> = ({
    data,
    index,
    onDetail,
    onPerbaiki,
        ...
        <div className="bg-white rounded-[3rem] border border-slate-200/60 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all group flex flex-col overflow-hidden relative">
            <div className="p-6 flex-1">
                <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-2xl flex items-center justify-center group-hover:scale-110 transition-all duration-500 shadow-lg shadow-purple-200">
                        <TrendingUp size={20} />
                    </div>
                    <div className="flex flex-col items-end gap-1">
                        <span className="text-[9px] font-black text-purple-600 bg-purple-50 px-3 py-1.5 rounded-full uppercase">
                            #{index}
                        </span>
                ...
                <h3 className="font-black text-slate-800 text-sm leading-tight mb-1 line-clamp-2 uppercase group-hover:text-purple-600 transition-colors">
                ...
                <div className="mb-3">
                    <div className="flex items-start gap-1.5">
                        <Building size={10} className="text-purple-400 mt-0.5 flex-shrink-0" />
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                    <span className="text-[9px] text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                        <Award size={10} />
                        {jenisKp}
                    </span>
                </div>
                ...
                <p className="text-[9px] text-slate-500 bg-slate-50 px-2 py-1 rounded-full inline-flex items-center gap-1 mb-3">
                    <FileCheck size={10} />
                    {availableFiles} dari {pangkatFileConfig.length} Berkas tersedia
                </p>
            ...
            <div className="bg-slate-50/50 p-4 space-y-2 border-t border-slate-100">
                <div className="flex gap-1.5 flex-wrap">
                    <button
                        onClick={onDetail}
                        className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border border-purple-200 text-purple-600 hover:bg-purple-600 hover:text-white hover:border-purple-600 transition-all"
                    >
                        <Eye size={12} /> Detail
                    </button>
                    ...
                    {status === "diterima" && (
                        <button
                            onClick={onUpload}
                            className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border border-purple-200 text-purple-600 hover:bg-purple-600 hover:text-white transition-all"
                        >
                ...
                <div className="flex gap-1 pt-1 flex-wrap">
                    {pangkatFileConfig.slice(0, 4).map(fileConfig => (
                        data[fileConfig.key] && data[fileConfig.key].trim() !== "" && (
                            <span key={fileConfig.key} className="text-[8px] px-2 py-0.5 rounded-full bg-purple-50 text-purple-600">
