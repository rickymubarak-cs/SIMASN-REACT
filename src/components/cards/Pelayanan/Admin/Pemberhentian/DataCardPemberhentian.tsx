// src/components/cards/Pelayanan/Admin/Pemberhentian/DataCardPemberhentian.tsx

import React, { useState } from 'react';
import { UserX, Clock, Eye, Edit, Ban, CheckCircle, Upload, FileCheck, Building, Loader } from 'lucide-react';
...
export const DataCardPemberhentian: React.FC<DataCardPemberhentianProps> = ({
    ...
    const handleTerimaClick = async () => {
        ...
        try {
            await onTerima?.(data?.layanan_pemberhentian_id, false);
        } finally {
    ...
    const getJenisPemberhentianBadge = () => {
        if (jenisPemberhentian === "BATAS USIA PENSIUN") {
            return <span className="text-[9px] text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1">Pensiun (BUP)</span>;
        } else if (jenisPemberhentian === "MENINGGAL DUNIA") {
            return <span className="text-[9px] text-gray-600 bg-gray-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1">Meninggal Dunia</span>;
        } else if (jenisPemberhentian === "PERMINTAAN SENDIRI") {
            return <span className="text-[9px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full inline-flex items-center gap-1">Permintaan Sendiri</span>;
        }
        return <span className="text-[9px] text-slate-500 bg-slate-50 px-2 py-0.5 rounded-full">{jenisPemberhentian}</span>;
    };
            ...
            <div className="p-6 flex-1">
                ...
                <div className="mb-3">
                    <div className="flex items-start gap-1.5">
                        <Building size={10} className="text-rose-400 mt-0.5 flex-shrink-0" />
            ...
            <div className="bg-slate-50/50 p-4 space-y-2 border-t border-slate-100">
                <div className="flex gap-1.5 flex-wrap">
                    <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border border-rose-200 text-rose-600 hover:bg-rose-600 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    {(status === "diterima" || status === "selesai") && (
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[9px] font-bold bg-white border transition-all hover:text-white ${status === "selesai"
                                    ? 'border-rose-200 text-rose-600 hover:bg-rose-600'
