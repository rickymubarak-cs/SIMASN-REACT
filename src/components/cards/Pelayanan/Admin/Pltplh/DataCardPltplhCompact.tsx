// src/components/cards/Pelayanan/Admin/Pltplh/DataCardPltplhCompact.tsx

import React, { useState } from 'react';
import { UserCog, Eye, Edit, Ban, CheckCircle, Upload, Loader, FileCheck } from 'lucide-react';
...
export const DataCardPltplhCompact: React.FC<DataCardPltplhCompactProps> = ({
    ...
    const handleTerimaClick = async () => {
        ...
        try {
            await onTerima?.(data?.layanan_pltplh_id, false);
        } finally {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 hover:shadow-lg hover:border-emerald-200 transition-all group">
            <div className="p-3">
                ...
                <div className="text-[9px] text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full inline-block mb-1">
                    {jenisPltplh}
                </div>
                ...
                <div className="flex gap-1 mt-2 pt-2 border-t border-slate-100">
                    <button
                        ...
                        className="flex-1 py-1.5 rounded-lg text-[9px] font-bold bg-slate-50 text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
                    >
