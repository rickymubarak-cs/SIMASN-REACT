...
import {
    X, User, Building, Paperclip, FileCheck, Clock, Info, Wallet, ChevronDown, ChevronUp,
    Mail, Gavel, Award, TrendingUp,  // FileCertificate diganti dengan Award atau FileCheck
    Eye, ExternalLink
} from 'lucide-react';
...
interface DetailModalGajiProps {
    isOpen: boolean;
    onClose: () => void;
    data: any;
}
...
const BASE_URL_BERKAS = "https://simasn.pontianak.go.id/assets/berkas/Layanan/Berkala/";

// Map icon untuk file (ganti FileCertificate dengan FileCheck atau Award)
...
export const DetailModalGaji: React.FC<DetailModalGajiProps> = ({
    isOpen,
    onClose,
    data
}) => {
    ...
    const availableFiles = gajiFileConfig
        ...
        <>
            ...
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
                ...
                <div className="relative bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn">
                    {/* Header */}
                    <div className="sticky top-0 bg-gradient-to-r from-teal-600 to-teal-700 text-white p-6 rounded-t-3xl">
                        <div className="flex justify-between items-start">
                            <div>
                                <h2 className="text-xl font-black flex items-center gap-2">
                                    <Wallet size={24} />
                                    Detail Kenaikan Gaji Berkala (KGB)
                                </h2>
                                <p className="text-teal-100 text-xs mt-1">Informasi lengkap pengajuan kenaikan gaji berkala</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="md:col-span-1 space-y-4">
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img src={fotoUrl} alt={namaLengkap} className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-teal-200 shadow-lg" />
                                    ) : (
                                        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 text-white flex items-center justify-center text-5xl font-black mx-auto border-4 border-teal-200 shadow-lg">
                                            {initials}
                                        </div>
                                    )}
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {visibleFiles.map((file) => {
                                        ...
                                        return (
                                            <div key={file.key} className="bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-teal-200 transition-all">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="flex gap-1">
                                                                <button onClick={() => window.open(file.url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-teal-400 hover:text-teal-600 text-[10px] font-medium">
                                                                    <ExternalLink size={10} className="inline mr-1" /> Buka
                                                                </button>
                                                                <button onClick={() => handlePreview(file.url, file.label)} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-teal-400 hover:text-teal-600 text-[10px] font-medium">
                                ...
                                {hasMoreFiles && (
                                    <button onClick={() => setShowAllFiles(!showAllFiles)} className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-medium text-teal-600 bg-teal-50 rounded-xl hover:bg-teal-100">
                                ...
                                {data.file_status_pelayanan && (
                                    <div className="mt-6">
                                        <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 mb-3"><FileCheck size={16} className="text-teal-600" /> Berkas Hasil</h4>
                                        <div className="bg-teal-50 rounded-xl p-3 border border-teal-200">
                                            <div className="flex items-start gap-3">
                                                <div className="p-2 rounded-lg bg-teal-100 text-teal-700"><FileCheck size={16} /></div>
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK Kenaikan Gaji Berkala</p>
                                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_status_pelayanan}</p>
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => window.open(data.file_status_pelayanan_url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-teal-400 hover:text-teal-600 text-[10px] font-medium">
                                                        <ExternalLink size={10} className="inline mr-1" /> Buka
                                                    </button>
                                                    <button onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK Kenaikan Gaji Berkala')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-teal-400 hover:text-teal-600 text-[10px] font-medium">
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
                                                </div>
                                            </div>
...
export default DetailModalGaji;