...
import {
    X, User, Building, Paperclip, FileCheck, Clock, Info, BookOpen, Calendar, Award,
    Eye, ExternalLink, MapPin, FileText
} from 'lucide-react';
...
const BASE_URL_BERKAS_ADMIN = "https://simasn.pontianak.go.id/assets/berkas/layanan_admin/kompetensi/";
...
export const DetailModalKompetensi: React.FC<DetailModalKompetensiProps> = ({
    ...
    const namaPelatihan = data.riw_kom_nama || "-";
    const penyelenggara = data.riw_kom_penyelenggara || "-";
    const nomorSertifikat = data.riw_kom_no || "-";
    const tanggalMulai = data.riw_kom_tmtm ? formatDateTimeId(data.riw_kom_tmtm) : "-";
    const tanggalSelesai = data.riw_kom_tmts ? formatDateTimeId(data.riw_kom_tmts) : "-";
    const tahun = data.riw_kom_tahun || "-";
    const jamPelatihan = data.riw_kom_jp || "-";

    const hasSertifikat = data.file_sertifikat && data.file_sertifikat.trim() !== "";
    const sertifikatUrl = hasSertifikat ? (data.file_sertifikat_url || `${BASE_URL_BERKAS}${data.file_sertifikat}`) : null;
        ...
        <>
            ...
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
                ...
                <div className="relative bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn">
                    {/* Header */}
                    <div className="sticky top-0 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white p-6 rounded-t-3xl">
                        <div className="flex justify-between items-start">
                            <div>
                                <h2 className="text-xl font-black flex items-center gap-2">
                                    <BookOpen size={24} />
                                    Detail Pengembangan Kompetensi
                                </h2>
                                <p className="text-cyan-100 text-xs mt-1">
                                    Informasi lengkap pengajuan pengembangan kompetensi / diklat
                                </p>
                            </div>
                    ...
                    <div className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img
                                            src={fotoUrl}
                                            alt={namaLengkap}
                                            className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-cyan-200 shadow-lg"
                                            onError={(e) => {
                                                e.currentTarget.style.display = "none";
                                                const parent = e.currentTarget.parentElement;
                                                if (parent) {
                                                    const fallback = document.createElement('div');
                                                    fallback.className = "w-32 h-32 rounded-full bg-gradient-to-br from-cyan-500 to-cyan-600 text-white flex items-center justify-center text-5xl font-black mx-auto border-4 border-cyan-200 shadow-lg";
                                        ...
                                        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-cyan-500 to-cyan-600 text-white flex items-center justify-center text-5xl font-black mx-auto border-4 border-cyan-200 shadow-lg">
                                            {initials}
                                        </div>
                                    )}
                            ...
                            {/* Detail Pelatihan Section */}
                            <div className="md:col-span-2">
                                <div className="bg-slate-50 rounded-2xl p-4 mb-4">
                                    <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 mb-3">
                                        <BookOpen size={16} className="text-cyan-600" /> Detail Pelatihan
                                    </h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                        <div>
                                            <p className="text-[10px] text-slate-400">Nama Pelatihan</p>
                                            <p className="font-medium text-slate-700 text-xs">{namaPelatihan}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Penyelenggara</p>
                                            <p className="font-medium text-slate-700 text-xs">{penyelenggara}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Nomor Sertifikat</p>
                                            <p className="font-medium text-slate-700 text-xs font-mono">{nomorSertifikat}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Tahun</p>
                                            <p className="font-medium text-slate-700 text-xs">{tahun}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Tanggal Pelaksanaan</p>
                                            <p className="font-medium text-slate-700 text-xs">{tanggalMulai} - {tanggalSelesai}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Jam Pelatihan (JP)</p>
                                            <p className="font-medium text-slate-700 text-xs">{jamPelatihan} Jam</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <span className="text-[10px] bg-slate-100 px-3 py-1 rounded-full font-bold">
                                        {hasSertifikat ? '1 berkas' : '0 berkas'}
                                    </span>
                                </div>

                                {hasSertifikat && sertifikatUrl && (
                                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-cyan-200 transition-all">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <p className="text-xs font-semibold text-slate-700">Sertifikat / Surat Tanda Tamat</p>
                                                        <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_sertifikat}</p>
                                                    </div>
                                                    <div className="flex gap-1">
                                                        <button
                                                            onClick={() => window.open(sertifikatUrl, '_blank')}
                                                            className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium"
                                                        >
                                                            <ExternalLink size={10} className="inline mr-1" /> Buka
                                                        </button>
                                                        <button
                                                            onClick={() => handlePreview(sertifikatUrl, 'Sertifikat / Surat Tanda Tamat')}
                                                            className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium"
                                                        >
                                ...
                                {data.file_status_pelayanan && (
                                    <div className="mt-6">
                                        <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 mb-3">
                                            <FileCheck size={16} className="text-cyan-600" /> Berkas Hasil
                                        </h4>
                                        <div className="bg-cyan-50 rounded-xl p-3 border border-cyan-200">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK / Surat Keterangan</p>
                                                ...
                                                <div className="flex gap-1">
                                                    ...
                                                    <button
                                                        onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK / Surat Keterangan')}
                                                        className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium"
                                                    >
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
