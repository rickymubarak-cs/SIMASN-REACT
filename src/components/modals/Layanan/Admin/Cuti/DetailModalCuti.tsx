...
import {
    ...
    ScrollText, ShieldCheck, BookOpen, GraduationCap
} from 'lucide-react';
...
export const DetailModalCuti: React.FC<DetailModalCutiProps> = ({
    ...
    const availableFiles = cutiFileConfig
        ...
        return (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl animate-fadeIn">
                    <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 text-white p-4 flex justify-between items-center">
                    ...
                    <div className="border-t border-slate-100 p-2.5 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button onClick={onClose} className="px-3 py-1 bg-cyan-600 text-white rounded-lg text-[10px] font-bold">Tutup</button>
                    </div>
                </div>
            </div>
        );
                        ...
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-cyan-100 text-xs mt-1">
                                    Informasi lengkap pengajuan usulan Cuti Pegawai
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-1 space-y-4">
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img src={fotoUrl} alt={namaLengkap} className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-cyan-200 shadow-lg" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                                    ) : (
                                ...
                                <div className="bg-slate-50 rounded-2xl p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-400">Jenis Cuti</p>
                                            <p className="font-medium text-slate-700 text-sm">{jenisCuti}</p>
                                        </div>
                                        <div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {visibleFiles.map((file) => {
                                        ...
                                        const colorClass = colorMap[file.color] || colorMap.cyan;

                                        return (
                                            <div key={file.key} className="bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-cyan-200 transition-all">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="flex gap-1">
                                                                <button onClick={() => window.open(file.url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium">
                                                                    <ExternalLink size={10} className="inline mr-1" /> Buka
                                                                </button>
                                                                <button onClick={() => handlePreview(file.url, file.label)} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium">
                                ...
                                {hasMoreFiles && (
                                    <button onClick={() => setShowAllFiles(!showAllFiles)} className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-medium text-cyan-600 bg-cyan-50 rounded-xl hover:bg-cyan-100 transition-colors">
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <div className="bg-cyan-50 rounded-xl p-3 border border-cyan-200">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK / Keputusan Cuti</p>
                                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_status_pelayanan}</p>
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => window.open(data.file_status_pelayanan_url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium">
                                                        <ExternalLink size={10} className="inline mr-1" /> Buka
                                                    </button>
                                                    <button onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK / Keputusan Cuti')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-cyan-400 hover:text-cyan-600 transition-all text-[10px] font-medium">
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
                                                </div>
                                            </div>
