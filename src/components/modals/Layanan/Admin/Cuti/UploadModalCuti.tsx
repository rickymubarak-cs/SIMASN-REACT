// src/components/modals/UploadModalCuti.tsx
import React, { useState } from 'react';
import { X, Upload, FileCheck, RefreshCcw, User, Calendar, Building, AlertCircle } from 'lucide-react';

interface UploadModalCutiProps {
    isOpen: boolean;
    onClose: () => void;
...
export const UploadModalCuti: React.FC<UploadModalCutiProps> = ({
    isOpen,
    onClose,
    onSubmit,
    data
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setUploading(true);
        try {
            await onSubmit(data.layanan_cuti_id || data.id, uploadFile);
            setUploadFile(null);
            setError(null);
            onClose();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-fadeIn">
                <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white p-6 rounded-t-3xl">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Upload size={24} />
                            <h2 className="text-xl font-black">Upload Berkas Hasil Cuti</h2>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-4 bg-cyan-50 rounded-xl p-4">
                        <p className="text-[10px] font-bold text-cyan-600 uppercase tracking-wider mb-3">Detail Usulan</p>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <p className="text-[9px] text-slate-400">Jenis Cuti</p>
                                <p className="text-sm font-bold text-cyan-700">{jenisCuti}</p>
                            </div>
                            <div>
                                <p className="text-[9px] text-slate-400">Tanggal Pengajuan</p>
                                <p className="text-sm font-bold text-cyan-700">{data.timestamp ? new Date(data.timestamp).toLocaleDateString('id-ID') : "-"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${error ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-cyan-500 bg-slate-50 hover:bg-cyan-50'}`} onClick={() => document.getElementById('uploadFileInputCuti')?.click()}>
                            ...
                            <input id="uploadFileInputCuti" type="file" accept=".pdf" className="hidden" onChange={(e) => { if (e.target.files?.[0]) { setError(null); setUploadFile(e.target.files[0]); } }} />
                        </div>
                        {uploadFile && (
                        ...
                        <p className="text-[10px] text-slate-400 mt-3">
                            Pastikan berkas yang diupload adalah SK / Keputusan Cuti hasil akhir yang sudah diverifikasi dan ditandatangani.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        <button type="submit" disabled={uploading || !uploadFile} className="flex-1 px-4 py-2 bg-cyan-500 text-white rounded-xl text-sm font-bold hover:bg-cyan-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
...
export default UploadModalCuti;