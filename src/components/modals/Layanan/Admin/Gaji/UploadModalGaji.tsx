// src/components/modals/Layanan/Admin/Gaji/UploadModalGaji.tsx

import React, { useState, useEffect } from 'react';
import { X, Upload, FileCheck, RefreshCcw, User, DollarSign, Building, AlertCircle } from 'lucide-react';
import { toast } from 'react-hot-toast';

interface UploadModalGajiProps {
    isOpen: boolean;
    onClose: () => void;
...
export const UploadModalGaji: React.FC<UploadModalGajiProps> = ({
    isOpen,
    onClose,
    onSubmit,
    onEdit,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!allowedTypes.includes(uploadFile.type)) {
            setError("Hanya file PDF, DOC, atau DOCX yang diperbolehkan");
            return;
        }
        ...
        try {
            if (mode === 'edit' && onEdit) {
                ...
                await onEdit(data.layanan_gaji_id || data.id, oldFileName, uploadFile);
                toast.success('Berkas berhasil diganti!', { id: loadingToast });
            } else {
                await onSubmit(data.layanan_gaji_id || data.id, uploadFile);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-fadeIn">
                <div className="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-6 rounded-t-3xl">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Upload size={24} />
                            <h2 className="text-xl font-black">
                                {mode === 'edit' ? 'Ganti Berkas Hasil Gaji Berkala' : 'Upload Berkas Hasil Gaji Berkala'}
                            </h2>
                        </div>
                        {mode === 'edit' && (
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-4 bg-teal-50 rounded-xl p-4">
                        <p className="text-[10px] font-bold text-teal-600 uppercase tracking-wider mb-3">Detail Usulan</p>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <p className="text-[9px] text-slate-400">Golongan Ruang</p>
                                <p className="text-sm font-bold text-teal-700">{golongan}</p>
                            </div>
                            <div>
                                <p className="text-[9px] text-slate-400">Tanggal Pengajuan</p>
                                <p className="text-sm font-bold text-teal-700">{data.timestamp ? new Date(data.timestamp).toLocaleDateString('id-ID') : "-"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-4">
                        <label className="block text-xs font-bold text-slate-600 mb-2">
                            {mode === 'edit' ? 'Berkas Baru' : 'Berkas Hasil'}
                        </label>
                        <div
                            className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer
                                ${error ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-teal-500 bg-slate-50 hover:bg-teal-50'}
                            `}
                            onClick={() => document.getElementById('uploadFileInputGaji')?.click()}
                        >
                            <Upload size={32} className={`mx-auto mb-2 ${error ? 'text-red-400' : 'text-slate-400'}`} />
                            <p className="text-xs font-medium text-slate-600">Klik untuk pilih file</p>
                            <p className="text-[9px] text-slate-400 mt-1">PDF, DOC, DOCX (Maksimal 10MB)</p>
                            <input
                                id="uploadFileInputGaji"
                                type="file"
                                accept=".pdf,.doc,.docx"
                                className="hidden"
                                onChange={(e) => {
                        ...
                        <p className="text-[10px] text-slate-400 mt-3">
                            {mode === 'edit'
                                ? 'Upload file baru untuk menggantikan berkas yang lama'
                                : 'Pastikan berkas yang diupload adalah SK Gaji Berkala hasil akhir yang sudah diverifikasi'
                            }
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className="flex-1 px-4 py-2 bg-teal-500 text-white rounded-xl text-sm font-bold hover:bg-teal-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
...
export default UploadModalGaji;