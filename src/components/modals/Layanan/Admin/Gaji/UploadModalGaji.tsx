// src/components/modals/UploadModalGaji.tsx
import React, { useState } from 'react';
import { X, Upload, FileCheck, RefreshCcw, User, DollarSign, Building, AlertCircle, TrendingUp } from 'lucide-react';
            ...
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-fadeIn">
                <div className="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-6 rounded-t-3xl">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Upload size={24} />
                            <h2 className="text-xl font-black">Upload Berkas Hasil Gaji Berkala</h2>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-4">
                        ...
                        <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${error ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-teal-500 bg-slate-50 hover:bg-teal-50'}`} onClick={() => document.getElementById('uploadFileInputGaji')?.click()}>
                            ...
                            <input id="uploadFileInputGaji" type="file" accept=".pdf" className="hidden" onChange={(e) => { if (e.target.files?.[0]) { setError(null); setUploadFile(e.target.files[0]); } }} />
                        </div>
                        {uploadFile && (
                        ...
                        <p className="text-[10px] text-slate-400 mt-3">
                            Pastikan berkas yang diupload adalah SK Gaji Berkala hasil akhir yang sudah diverifikasi dan ditandatangani.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        <button type="submit" disabled={uploading || !uploadFile} className="flex-1 px-4 py-2 bg-teal-500 text-white rounded-xl text-sm font-bold hover:bg-teal-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
