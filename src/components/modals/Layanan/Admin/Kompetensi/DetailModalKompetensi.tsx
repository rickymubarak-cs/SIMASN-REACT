// src/components/modals/DetailModalKompetensi.tsx
import React, { useState } from 'react';
import {
    X, User, Building, Paperclip, FileCheck, Clock, Info, Award, ChevronDown, ChevronUp,
    Mail, FileSignature, FileText, Briefcase, IdCard, Eye, ExternalLink, Calendar, FileBadge
} from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';
import { formatDateTimeId, getInitials } from '../../../../../utils/formatters';
import { kompetensiFileConfig } from '../../../../../service/kompetensiService';

interface DetailModalKompetensiProps {
    isOpen: boolean;
    onClose: () => void;
    data: any;
}
...
const BASE_URL_BERKAS = "https://simasn.pontianak.go.id/assets/berkas/Layanan/Kompetensi/";
...
export const DetailModalKompetensi: React.FC<DetailModalKompetensiProps> = ({
    isOpen,
    onClose,
    data
}) => {
    ...
    const availableFiles = kompetensiFileConfig
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl animate-fadeIn">
                    ...
                    <div className="border-t border-slate-100 p-2.5 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button onClick={onClose} className="px-3 py-1 bg-orange-600 text-white rounded-lg text-[10px] font-bold">Tutup</button>
                    </div>
                </div>
            </div>
        );
        ...
        <>
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
                ...
                <div className="relative bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn">
                    <div className="sticky top-0 bg-gradient-to-r from-orange-600 to-orange-700 text-white p-6 rounded-t-3xl">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-orange-100 text-xs mt-1">
                                    Informasi lengkap pengajuan usulan Kompetensi / Sertifikasi
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-1 space-y-4">
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img src={fotoUrl} alt={namaLengkap} className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-orange-200 shadow-lg" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                                    ) : (
                                        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 text-white flex items-center justify-center text-5xl font-black mx-auto border-4 border-orange-200 shadow-lg">
                                            {initials}
                                        </div>
                                    )}
                                ...
                                <div className="bg-slate-50 rounded-2xl p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-400">Jenis Kompetensi</p>
                                            <p className="font-medium text-slate-700 text-sm">{jenisKompetensi}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Tahun</p>
                                            <p className="font-medium text-slate-700 text-sm">{tahun}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Jam Pelajaran</p>
                                            <p className="font-medium text-slate-700 text-sm">{jamPelajaran} JP</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {visibleFiles.map((file) => {
                                        ...
                                        const colorClass = colorMap[file.color] || colorMap.orange;

                                        return (
                                            <div key={file.key} className="bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-orange-200 transition-all">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="flex gap-1">
                                                                <button onClick={() => window.open(file.url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-orange-400 hover:text-orange-600 transition-all text-[10px] font-medium">
                                                                    <ExternalLink size={10} className="inline mr-1" /> Buka
                                                                </button>
                                                                <button onClick={() => handlePreview(file.url, file.label)} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-orange-400 hover:text-orange-600 transition-all text-[10px] font-medium">
                                ...
                                {hasMoreFiles && (
                                    <button onClick={() => setShowAllFiles(!showAllFiles)} className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-medium text-orange-600 bg-orange-50 rounded-xl hover:bg-orange-100 transition-colors">
                                ...
                                <div className="mt-6 bg-slate-50 rounded-xl p-4">
                                    <h4 className="text-xs font-black text-slate-700 mb-3">Detail Kompetensi</h4>
                                    <div className="space-y-3">
                                        <div>
                                            <p className="text-[10px] text-slate-400">Nama Kompetensi</p>
                                            <p className="text-sm font-medium text-slate-700">{namaKompetensi}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Penyelenggara</p>
                                            <p className="text-sm text-slate-600">{penyelenggara}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-400">Nomor Sertifikat</p>
                                            <p className="text-sm font-mono text-slate-600">{nomorSertifikat}</p>
                                        </div>
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <p className="text-[10px] text-slate-400">Tanggal Mulai</p>
                                                <p className="text-sm text-slate-600">{formatDateTimeId(tanggalMulai)}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-slate-400">Tanggal Selesai</p>
                                                <p className="text-sm text-slate-600">{formatDateTimeId(tanggalSelesai)}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {data.file_status_pelayanan && (
                                    <div className="mt-6">
                                        <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 mb-3">
                                            <FileCheck size={16} className="text-orange-600" /> Berkas Hasil
                                        </h4>
                                        <div className="bg-orange-50 rounded-xl p-3 border border-orange-200">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK / Sertifikat Kompetensi</p>
                                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_status_pelayanan}</p>
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => window.open(data.file_status_pelayanan_url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-orange-400 hover:text-orange-600 transition-all text-[10px] font-medium">
                                                        <ExternalLink size={10} className="inline mr-1" /> Buka
                                                    </button>
                                                    <button onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK / Sertifikat Kompetensi')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-orange-400 hover:text-orange-600 transition-all text-[10px] font-medium">
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
                                                </div>
                                            </div>
...
export default DetailModalKompetensi;