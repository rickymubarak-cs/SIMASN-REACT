// src/components/modals/DetailModalPemberhentian.tsx
import React, { useState } from 'react';
import {
    X, User, Building, Paperclip, FileCheck, Clock, Info, UserX, ChevronDown, ChevronUp,
    Mail, FileSignature, FileText, Award, IdCard, Eye, ExternalLink, Database, DollarSign,
    ScrollText, ShieldCheck, BookOpen, GraduationCap, Briefcase, Heart, Users, Camera
} from 'lucide-react';
...
const BASE_URL_BERKAS = "https://simasn.pontianak.go.id/assets/berkas/Layanan/Pemberhentian/";
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl animate-fadeIn">
                    ...
                    <div className="border-t border-slate-100 p-2.5 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button onClick={onClose} className="px-3 py-1 bg-rose-600 text-white rounded-lg text-[10px] font-bold">Tutup</button>
                    </div>
                </div>
            </div>
        );
        ...
        <>
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
                ...
                <div className="relative bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn">
                    <div className="sticky top-0 bg-gradient-to-r from-rose-600 to-rose-700 text-white p-6 rounded-t-3xl">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-rose-100 text-xs mt-1">
                                    Informasi lengkap pengajuan usulan Pemberhentian ASN
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-1 space-y-4">
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img src={fotoUrl} alt={namaLengkap} className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-rose-200 shadow-lg" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                                    ) : (
                                ...
                                <div className="bg-slate-50 rounded-2xl p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium text-slate-700 text-sm">{jenisPemberhentian}</p>
                                        </div>
                                        <div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {visibleFiles.map((file) => {
                                        ...
                                        const colorClass = colorMap[file.color] || colorMap.rose;
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="flex gap-1">
                                                                <button onClick={() => window.open(file.url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-rose-400 hover:text-rose-600 transition-all text-[10px] font-medium">
                                                                    <ExternalLink size={10} className="inline mr-1" /> Buka
                                                                </button>
                                                                <button onClick={() => handlePreview(file.url, file.label)} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-rose-400 hover:text-rose-600 transition-all text-[10px] font-medium">
                                ...
                                {hasMoreFiles && (
                                    <button onClick={() => setShowAllFiles(!showAllFiles)} className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-medium text-rose-600 bg-rose-50 rounded-xl hover:bg-rose-100 transition-colors">
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <div className="bg-rose-50 rounded-xl p-3 border border-rose-200">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK Pemberhentian ASN</p>
                                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_status_pelayanan}</p>
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => window.open(data.file_status_pelayanan_url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-rose-400 hover:text-rose-600 transition-all text-[10px] font-medium">
                                                        <ExternalLink size={10} className="inline mr-1" /> Buka
                                                    </button>
                                                    <button onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK Pemberhentian ASN')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-rose-400 hover:text-rose-600 transition-all text-[10px] font-medium">
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
                                                </div>
                                            </div>
