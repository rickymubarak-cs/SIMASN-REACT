// src/components/modals/Layanan/Admin/Pltplh/UploadModalPltplh.tsx

import React, { useState, useEffect } from 'react';
import { X, Upload, FileCheck, RefreshCcw, User, UserCog, Building, AlertCircle } from 'lucide-react';
import { toast } from 'react-hot-toast';

interface UploadModalPltplhProps {
    isOpen: boolean;
    onClose: () => void;
...
export const UploadModalPltplh: React.FC<UploadModalPltplhProps> = ({
    isOpen,
    onClose,
    onSubmit,
    onEdit,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === 'edit' && onEdit) {
                ...
                await onEdit(data.layanan_pltplh_id || data.id, oldFileName, uploadFile);
                toast.success('Berkas berhasil diganti!', { id: loadingToast });
            } else {
                await onSubmit(data.layanan_pltplh_id || data.id, uploadFile);
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-black">
                                {mode === 'edit' ? 'Ganti Berkas Hasil PLT/PLH' : 'Upload Berkas Hasil PLT/PLH'}
                            </h2>
                        </div>
                        {mode === 'edit' && (
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    <div className="mb-4 bg-slate-50 rounded-xl p-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                <UserCog size={14} className="text-slate-400 flex-shrink-0" />
                                <div className="flex-1">
                    ...
                    <div className="mb-4 bg-emerald-50 rounded-xl p-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <p className="text-[9px] text-slate-400">Jenis PLT/PLH</p>
                                <p className="text-sm font-bold text-emerald-700">{jenisPltplh}</p>
                            </div>
                            <div>
                    ...
                    <div className="mb-4">
                        ...
                        <div
                            className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer
                                ${error ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-emerald-500 bg-slate-50 hover:bg-emerald-50'}
                            `}
                            onClick={() => document.getElementById('uploadFileInputPltplh')?.click()}
                        >
                        ...
                        <p className="text-[10px] text-slate-400 mt-3">
                            {mode === 'edit'
                                ...
                                : 'Pastikan berkas yang diupload adalah SK / Keputusan PLT/PLH hasil akhir yang sudah diverifikasi'
                            }
                        </p>
                    </div>
...
export default UploadModalPltplh;