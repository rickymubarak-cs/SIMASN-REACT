// src/components/modals/DetailModalDataPerubahan.tsx
import React, { useState } from 'react';
import {
    X, User, Building, Paperclip, FileCheck, Clock, Info, Database, ChevronDown, ChevronUp,
    Mail, FileSignature, FileText, Briefcase, IdCard, Eye, ExternalLink, Calendar
} from 'lucide-react';
import { StatusBadge } from '../../../../common/StatusBadge';
import { formatDateTimeId, getInitials } from '../../../../../utils/formatters';
import { dataFileConfig } from '../../../../../service/dataService';

interface DetailModalDataPerubahanProps {
    isOpen: boolean;
    onClose: () => void;
    data: any;
}
...
const BASE_URL_BERKAS = "https://simasn.pontianak.go.id/assets/berkas/Layanan/Data/";
...
export const DetailModalDataPerubahan: React.FC<DetailModalDataPerubahanProps> = ({
    isOpen,
    onClose,
    data
}) => {
    ...
    const availableFiles = dataFileConfig
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                ...
                <div className="relative bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] overflow-hidden shadow-2xl animate-fadeIn">
                    ...
                    <div className="p-3 bg-slate-100 min-h-[400px] max-h-[calc(85vh-100px)] overflow-auto relative">
                        {isLoading && !loadError && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-100 z-10">
                                <div className="w-10 h-10 border-4 border-slate-500 border-t-transparent rounded-full animate-spin mb-3"></div>
                    ...
                    <div className="border-t border-slate-100 p-2.5 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button onClick={onClose} className="px-3 py-1 bg-slate-600 text-white rounded-lg text-[10px] font-bold">Tutup</button>
                    </div>
                </div>
            </div>
        );
        ...
        <>
            <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isOpen ? 'visible' : 'invisible'}`}>
                ...
                <div className="relative bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-fadeIn">
                    <div className="sticky top-0 bg-gradient-to-r from-slate-600 to-slate-700 text-white p-6 rounded-t-3xl">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-slate-100 text-xs mt-1">
                                    Informasi lengkap pengajuan usulan Perubahan Data Pegawai
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="md:col-span-1 space-y-4">
                                <div className="text-center">
                                    {fotoUrl ? (
                                        <img src={fotoUrl} alt={namaLengkap} className="w-32 h-32 rounded-full object-cover mx-auto border-4 border-slate-200 shadow-lg" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                                    ) : (
                                        <div className="w-32 h-32 rounded-full bg-gradient-to-br from-slate-500 to-slate-600 text-white flex items-center justify-center text-5xl font-black mx-auto border-4 border-slate-200 shadow-lg">
                                            {initials}
                                        </div>
                                    )}
                                ...
                                <div className="bg-slate-50 rounded-2xl p-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-400">Jenis Perubahan Data</p>
                                            <p className="font-medium text-slate-700 text-sm">{jenisLayanan}</p>
                                        </div>
                                        <div>
                                ...
                                <div className="bg-slate-50 rounded-2xl p-4">
                                    ...
                                    {ketPerbaikan !== "-" && ketPerbaikan && (
                                        <div className="mt-3 pt-3 border-t border-slate-200">
                                            <p className="text-[10px] text-amber-600 font-semibold">Keterangan Perbaikan</p>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {visibleFiles.map((file) => {
                                        ...
                                        const colorClass = colorMap[file.color] || colorMap.slate;

                                        return (
                                            <div key={file.key} className="bg-slate-50 rounded-xl p-3 border border-slate-100 hover:border-slate-300 transition-all">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="flex gap-1">
                                                                <button onClick={() => window.open(file.url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-slate-400 hover:text-slate-700 transition-all text-[10px] font-medium">
                                                                    <ExternalLink size={10} className="inline mr-1" /> Buka
                                                                </button>
                                                                <button onClick={() => handlePreview(file.url, file.label)} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-slate-400 hover:text-slate-700 transition-all text-[10px] font-medium">
                                ...
                                {hasMoreFiles && (
                                    <button onClick={() => setShowAllFiles(!showAllFiles)} className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-medium text-slate-600 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
                                ...
                                {data.file_status_pelayanan && (
                                    <div className="mt-6">
                                        <h4 className="text-sm font-black text-slate-800 flex items-center gap-2 mb-3">
                                            <FileCheck size={16} className="text-slate-600" /> Berkas Hasil
                                        </h4>
                                        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1">
                                                    <p className="text-xs font-semibold text-slate-700">SK / Keputusan Perubahan Data</p>
                                                    <p className="text-[10px] text-slate-400 font-mono mt-0.5">{data.file_status_pelayanan}</p>
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => window.open(data.file_status_pelayanan_url, '_blank')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-slate-400 hover:text-slate-700 transition-all text-[10px] font-medium">
                                                        <ExternalLink size={10} className="inline mr-1" /> Buka
                                                    </button>
                                                    <button onClick={() => handlePreview(data.file_status_pelayanan_url, 'SK / Keputusan Perubahan Data')} className="px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-slate-400 hover:text-slate-700 transition-all text-[10px] font-medium">
                                                        <Eye size={10} className="inline mr-1" /> Preview
                                                    </button>
                                                </div>
                                            </div>
...
export default DetailModalDataPerubahan;