// src/components/modals/UploadModalDataPerubahan.tsx
import React, { useState } from 'react';
import { X, Upload, FileCheck, RefreshCcw, User, Database, Building, AlertCircle } from 'lucide-react';

interface UploadModalDataPerubahanProps {
    isOpen: boolean;
    onClose: () => void;
...
export const UploadModalDataPerubahan: React.FC<UploadModalDataPerubahanProps> = ({
    isOpen,
    onClose,
    onSubmit,
    data
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setUploading(true);
        try {
            await onSubmit(data.layanan_data_id || data.id, uploadFile);
            setUploadFile(null);
            setError(null);
            onClose();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-fadeIn">
                <div className="bg-gradient-to-r from-slate-500 to-slate-600 text-white p-6 rounded-t-3xl">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Upload size={24} />
                            <h2 className="text-xl font-black">Upload Berkas Hasil Perubahan Data</h2>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="mb-4 bg-slate-100 rounded-xl p-4">
                        <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider mb-3">Detail Usulan</p>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <p className="text-[9px] text-slate-400">Jenis Perubahan</p>
                                <p className="text-sm font-bold text-slate-700">{jenisLayanan}</p>
                            </div>
                            <div>
                                <p className="text-[9px] text-slate-400">Tanggal Pengajuan</p>
                                <p className="text-sm font-bold text-slate-700">{data.timestamp ? new Date(data.timestamp).toLocaleDateString('id-ID') : "-"}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-4">
                        ...
                        <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${error ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-slate-500 bg-slate-50 hover:bg-slate-100'}`} onClick={() => document.getElementById('uploadFileInputData')?.click()}>
                            ...
                            <input id="uploadFileInputData" type="file" accept=".pdf" className="hidden" onChange={(e) => { if (e.target.files?.[0]) { setError(null); setUploadFile(e.target.files[0]); } }} />
                        </div>
                        {uploadFile && (
                        ...
                        <p className="text-[10px] text-slate-400 mt-3">
                            Pastikan berkas yang diupload adalah SK / Keputusan Perubahan Data hasil akhir yang sudah diverifikasi.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3 mt-6">
                        ...
                        <button type="submit" disabled={uploading || !uploadFile} className="flex-1 px-4 py-2 bg-slate-600 text-white rounded-xl text-sm font-bold hover:bg-slate-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
...
export default UploadModalDataPerubahan;