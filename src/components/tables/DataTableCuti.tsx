...
import { Eye, Edit, Ban, CheckCircle, Upload, ChevronUp, ChevronDown, Calendar } from 'lucide-react';
                ...
                <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('status')} className="flex items-center gap-1 hover:text-cyan-600">
                                ...
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-1 justify-center">
                                            <button onClick={() => onDetail(item)} className="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-cyan-100 hover:text-cyan-600 transition-all" title="Detail">
                                                <Eye size={14} />
                                            </button>
                                            {status === "pengajuan" && (
                                                <>
                                                    ...
                                                    <button onClick={() => onTerima(item.layanan_cuti_id, false)} className="p-1.5 rounded-lg bg-green-100 text-green-600 hover:bg-green-600 hover:text-white transition-all" title="Terima">
                                                        <CheckCircle size={14} />
                                                    </button>
                                                </>
                                            )}
                                            {status === "diterima" && (
                                                <button onClick={() => onUpload(item)} className="p-1.5 rounded-lg bg-cyan-100 text-cyan-600 hover:bg-cyan-600 hover:text-white transition-all" title="Upload Berkas">
                                                    <Upload size={14} />
                                                </button>
                                            )}
                                        </div>
