// src/components/tables/DataTableCuti.tsx

import React, { useState } from 'react';
import { Eye, Edit, Ban, CheckCircle, Upload, ChevronUp, ChevronDown, Loader, FileCheck, Calendar } from 'lucide-react';
import { StatusBadge } from '../common/StatusBadge';
import { formatDateTimeId } from '../../utils/formatters';

interface DataTableCutiProps {
    data: any[];
    startIndex: number;
...
export const DataTableCuti: React.FC<DataTableCutiProps> = ({
    data,
    startIndex,
    onDetail,
    onPerbaiki,
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'tanggal':
                    ...
                    break;
                case 'status':
                    aVal = a.layanan_cuti_status || '';
                    bVal = b.layanan_cuti_status || '';
                    break;
                case 'jenis':
                    aVal = a.jenis_cuti || '';
                    bVal = b.jenis_cuti || '';
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nama')} className="flex items-center gap-1 hover:text-cyan-600">
                                    Nama Pegawai {getSortIcon('nama')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nip')} className="flex items-center gap-1 hover:text-cyan-600">
                                    NIP {getSortIcon('nip')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('unit')} className="flex items-center gap-1 hover:text-cyan-600">
                                    Unit Kerja {getSortIcon('unit')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('jenis')} className="flex items-center gap-1 hover:text-cyan-600">
                                    Jenis Cuti {getSortIcon('jenis')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('tanggal')} className="flex items-center gap-1 hover:text-cyan-600">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredAndSortedData.map((item, idx) => {
                            const status = item.layanan_cuti_status || "pengajuan";
                            const namaLengkap = getNamaLengkap(item);
                            const isLoading = actionLoading?.id === item.layanan_cuti_id;
                                ...
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-1 justify-center">
                                            <button
                                                onClick={() => handleAction('detail', item.layanan_cuti_id, () => onDetail(item))}
                                                disabled={isLoading}
                                                className="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-cyan-100 hover:text-cyan-600 transition-all disabled:opacity-50"
                                                title="Detail"
                                            >
                                                <Eye size={14} />
                                            </button>
                                            ...
                                            {status === "pengajuan" && (
                                                <>
                                                    <button
                                                        onClick={() => handleAction('perbaiki', item.layanan_cuti_id, () => onPerbaiki(item))}
                                                        disabled={isLoading}
                                                    ...
                                                    <button
                                                        onClick={() => handleAction('tolak', item.layanan_cuti_id, () => onTolak(item))}
                                                        disabled={isLoading}
                                                    ...
                                                    <button
                                                        onClick={() => handleAction('terima', item.layanan_cuti_id, () => onTerima(item.layanan_cuti_id, false))}
                                                        disabled={isLoading}
                                            ...
                                            {(status === "diterima" || status === "selesai") && (
                                                <button
                                                    onClick={() => handleAction('upload', item.layanan_cuti_id, () => onUpload(item))}
                                                    disabled={isLoading}
                                                    className={`p-1.5 rounded-lg transition-all disabled:opacity-50 ${status === "selesai"
                                                            ? 'bg-cyan-100 text-cyan-600 hover:bg-cyan-600 hover:text-white'
            ...
            {filteredAndSortedData.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-slate-500">Tidak ada data Cuti yang ditemukan</p>
                </div>
            )}
        </div>
    );
