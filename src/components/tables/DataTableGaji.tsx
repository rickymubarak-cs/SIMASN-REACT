...
import { Eye, Edit, Ban, CheckCircle, Upload, ChevronUp, ChevronDown, Loader, FileCheck, DollarSign } from 'lucide-react';
                ...
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredAndSortedData.map((item, idx) => {
                            ...
                            const isLoading = actionLoading?.id === item.layanan_gaji_id;
                                ...
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-1 justify-center">
                                            <button
                                                onClick={() => handleAction('detail', item.layanan_gaji_id, () => onDetail(item))}
                                                disabled={isLoading}
                                                className="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-teal-100 hover:text-teal-600 transition-all disabled:opacity-50"
                                                title="Detail"
                                            >
                                                <Eye size={14} />
                                            </button>
                                            ...
                                            {status === "pengajuan" && (
                                                <>
                                                    <button
                                                        onClick={() => handleAction('perbaiki', item.layanan_gaji_id, () => onPerbaiki(item))}
                                                        disabled={isLoading}
                                                    ...
                                                    <button
                                                        onClick={() => handleAction('tolak', item.layanan_gaji_id, () => onTolak(item))}
                                                        disabled={isLoading}
                                                    ...
                                                    <button
                                                        onClick={() => handleAction('terima', item.layanan_gaji_id, () => onTerima(item.layanan_gaji_id, false))}
                                                        disabled={isLoading}
                                            ...
                                            {(status === "diterima" || status === "selesai") && (
                                                <button
                                                    onClick={() => handleAction('upload', item.layanan_gaji_id, () => onUpload(item))}
                                                    disabled={isLoading}
