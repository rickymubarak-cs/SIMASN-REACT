// src/components/tables/DataTableGaji.tsx
import React, { useState } from 'react';
import { Eye, Edit, Ban, CheckCircle, Upload, ChevronUp, ChevronDown, DollarSign } from 'lucide-react';
import { StatusBadge } from '../common/StatusBadge';
import { formatDateTimeId } from '../../utils/formatters';

interface DataTableGajiProps {
    data: any[];
    startIndex: number;
...
type SortField = 'nama' | 'nip' | 'unit' | 'tanggal' | 'status' | 'golongan';
type SortOrder = 'asc' | 'desc';

export const DataTableGaji: React.FC<DataTableGajiProps> = ({
    data,
    startIndex,
    onDetail,
    onPerbaiki,
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'tanggal':
                    ...
                    break;
                case 'status':
                    aVal = a.layanan_berkala_status || '';
                    bVal = b.layanan_berkala_status || '';
                    break;
                case 'golongan':
                    aVal = a.gol_id || '';
                    bVal = b.gol_id || '';
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nama')} className="flex items-center gap-1 hover:text-teal-600">
                                    Nama Pegawai {getSortIcon('nama')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nip')} className="flex items-center gap-1 hover:text-teal-600">
                                    NIP {getSortIcon('nip')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('unit')} className="flex items-center gap-1 hover:text-teal-600">
                                    Unit Kerja {getSortIcon('unit')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('golongan')} className="flex items-center gap-1 hover:text-teal-600">
                                    Golongan {getSortIcon('golongan')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('tanggal')} className="flex items-center gap-1 hover:text-teal-600">
                                    Tanggal {getSortIcon('tanggal')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('status')} className="flex items-center gap-1 hover:text-teal-600">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredAndSortedData.map((item, idx) => {
                            const status = item.layanan_berkala_status || "pengajuan";
                                ...
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-1 justify-center">
                                            <button onClick={() => onDetail(item)} className="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-teal-100 hover:text-teal-600 transition-all" title="Detail">
                                                <Eye size={14} />
                                            </button>
                                            {status === "pengajuan" && (
                                                <>
                                                    ...
                                                    <button onClick={() => onTerima(item.layanan_gaji_id, false)} className="p-1.5 rounded-lg bg-green-100 text-green-600 hover:bg-green-600 hover:text-white transition-all" title="Terima">
                                                        <CheckCircle size={14} />
                                                    </button>
                                                </>
                                            )}
                                            {status === "diterima" && (
                                                <button onClick={() => onUpload(item)} className="p-1.5 rounded-lg bg-teal-100 text-teal-600 hover:bg-teal-600 hover:text-white transition-all" title="Upload Berkas">
                                                    <Upload size={14} />
                                                </button>
                                            )}
                                        </div>
            ...
            {filteredAndSortedData.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-slate-500">Tidak ada data Gaji Berkala yang ditemukan</p>
                </div>
            )}
        </div>
    );
