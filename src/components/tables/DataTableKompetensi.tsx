// src/components/tables/DataTableKompetensi.tsx
import React, { useState } from 'react';
import { Eye, Edit, Ban, CheckCircle, Upload, ChevronUp, ChevronDown, Award } from 'lucide-react';
import { StatusBadge } from '../common/StatusBadge';
import { formatDateTimeId } from '../../utils/formatters';

interface DataTableKompetensiProps {
    data: any[];
    startIndex: number;
...
type SortField = 'nama' | 'nip' | 'unit' | 'tanggal' | 'status' | 'jenis' | 'tahun';
type SortOrder = 'asc' | 'desc';

export const DataTableKompetensi: React.FC<DataTableKompetensiProps> = ({
    data,
    startIndex,
    onDetail,
    onPerbaiki,
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'status':
                    ...
                    break;
                case 'jenis':
                    aVal = a.riw_kom_jenis || '';
                    bVal = b.riw_kom_jenis || '';
                    break;
                case 'tahun':
                    aVal = a.riw_kom_tahun || '';
                    bVal = b.riw_kom_tahun || '';
                    break;
                default:
                    return 0;
            }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nama')} className="flex items-center gap-1 hover:text-orange-600">
                                    Nama Pegawai {getSortIcon('nama')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('nip')} className="flex items-center gap-1 hover:text-orange-600">
                                    NIP {getSortIcon('nip')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('unit')} className="flex items-center gap-1 hover:text-orange-600">
                                    Unit Kerja {getSortIcon('unit')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('jenis')} className="flex items-center gap-1 hover:text-orange-600">
                                    Jenis {getSortIcon('jenis')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('tahun')} className="flex items-center gap-1 hover:text-orange-600">
                                    Tahun {getSortIcon('tahun')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                <button onClick={() => handleSort('tanggal')} className="flex items-center gap-1 hover:text-orange-600">
                                    Tanggal Pengajuan {getSortIcon('tanggal')}
                                </button>
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                                ...
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex gap-1 justify-center">
                                            <button onClick={() => onDetail(item)} className="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-orange-100 hover:text-orange-600 transition-all" title="Detail">
                                                <Eye size={14} />
                                            </button>
                                            {status === "pengajuan" && (
                                                <>
                                                    ...
                                                    <button onClick={() => onTerima(item.komp_id, false)} className="p-1.5 rounded-lg bg-green-100 text-green-600 hover:bg-green-600 hover:text-white transition-all" title="Terima">
                                                        <CheckCircle size={14} />
                                                    </button>
                                                </>
                                            )}
                                            {status === "diterima" && (
                                                <button onClick={() => onUpload(item)} className="p-1.5 rounded-lg bg-orange-100 text-orange-600 hover:bg-orange-600 hover:text-white transition-all" title="Upload Berkas">
                                                    <Upload size={14} />
                                                </button>
                                            )}
                                        </div>
            ...
            {filteredAndSortedData.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-slate-500">Tidak ada data Kompetensi yang ditemukan</p>
                </div>
            )}
        </div>
    );
