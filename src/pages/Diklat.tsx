// src/pages/Diklat.tsx
import React, { useState, useEffect } from 'react';
import { AlertCircle, BookOpen } from 'lucide-react';
import { useDiklatData } from '../hooks/useDiklatData';
import { Navbar } from '../components/layout/Navbar';
import { DashboardSearchBar } from '../components/layout/DashboardSearchBar';
import { DataCardDiklat } from '../components/cards/Pelayanan/Admin/Diklat/DataCardDiklat';
import { DataCardDiklatCompact } from '../components/cards/Pelayanan/Admin/Diklat/DataCardDiklatCompact';
import { DataCardDiklatDetailed } from '../components/cards/Pelayanan/Admin/Diklat/DataCardDiklatDetailed';
import { DataTableDiklat } from '../components/tables/DataTableDiklat';
import { DetailModalDiklat } from '../components/modals/Layanan/Admin/Diklat/DetailModalDiklat';
import { ActionModal } from '../components/modals/ActionModal';
import { UploadModalDiklat } from '../components/modals/Layanan/Admin/Diklat/UploadModalDiklat';
import { SkeletonLoading } from '../components/common/SkeletonLoading';
import { diklatService } from '../service/diklatService';

type ViewMode = 'standard' | 'compact' | 'detailed' | 'table';

interface DiklatProps {
    activeTab: string;
    onTabChange: (tab: string) => void;
}

export default function Diklat({ activeTab, onTabChange }: DiklatProps) {
    ...
    const { data, loading, error, refreshData } = useDiklatData();
    ...
    const filteredData = data.filter(item => {
        ...
        const namaDiklat = item.nama_usulan_diklat || "";
        ...
        return (
            ...
            namaDiklat.toLowerCase().includes(searchLower)
        );
    });
    ...
    const handlePerbaiki = async (id: string, keterangan: string) => {
        try {
            await diklatService.updateStatus(id, 'perbaikan', keterangan);
            refreshData();
    ...
    const handleTolak = async (id: string, alasan: string) => {
        try {
            await diklatService.updateStatus(id, 'ditolak', alasan);
            refreshData();
    ...
    const handleTerima = async (id: string, isTembusan: boolean) => {
        try {
            await diklatService.updateStatus(id, isTembusan ? 'selesai' : 'diterima');
            refreshData();
    ...
    const handleUpload = async (id: string, file: File) => {
        try {
            await diklatService.uploadBerkas(id, file);
            refreshData();
    ...
    const renderContent = () => {
        ...
        if (paginatedData.length === 0) {
            return <div className="text-center py-20 bg-white rounded-3xl shadow-sm"><div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-8"><BookOpen size={40} className="text-slate-300" /></div><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Belum Ada Usulan Diklat</h3><p className="text-slate-500 text-sm mt-2">Saat ini belum terdapat pengajuan usulan Diklat / Pelatihan{unitKerjaName ? ` untuk unit kerja ${unitKerjaName}` : ''}.</p><button onClick={refreshData} className="mt-6 px-6 py-2 bg-sky-500 text-white rounded-xl text-sm font-bold hover:bg-sky-600">Refresh Data</button></div>;
        }

        switch (viewMode) {
            case 'compact':
                return <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">{paginatedData.map((item, idx) => <DataCardDiklatCompact key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'detailed':
                return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{paginatedData.map((item, idx) => <DataCardDiklatDetailed key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'table':
                return <DataTableDiklat data={paginatedData} startIndex={(currentPage - 1) * itemsPerPage} onDetail={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />;
            default:
                return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{paginatedData.map((item, idx) => <DataCardDiklat key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9]">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                <div className="bg-gradient-to-r from-sky-600 to-sky-700 rounded-3xl p-6 text-white">
                    <div className="mb-4">
                        <h1 className="text-2xl font-black mb-1">Diklat & Pelatihan</h1>
                        <p className="text-sky-100 text-sm">Kelola dan pantau pengajuan usulan Diklat dan Pelatihan Pegawai</p>
                    </div>
                    <DashboardSearchBar searchTerm={searchTerm} onSearchChange={(value) => { setSearchTerm(value); setCurrentPage(1); }} onSearch={() => setCurrentPage(1)} loading={loading} perangkatDaerah={unitKerjaName} onPerangkatDaerahChange={setUnitKerjaName} showUnitFilter={true} onFilter={refreshData} viewMode={viewMode} onViewModeChange={(mode) => { setViewMode(mode); setCurrentPage(1); }} itemCount={filteredData.length} variant="diklat" placeholder="Cari berdasarkan NIP, Nama, atau Nama Diklat..." buttonText="Cari" />
                </div>
                {renderContent()}
            ...
            <DetailModalDiklat isOpen={modalState.detail} onClose={() => setModalState(prev => ({ ...prev, detail: false }))} data={selectedData} />
            ...
            <UploadModalDiklat isOpen={modalState.upload} onClose={() => setModalState(prev => ({ ...prev, upload: false }))} onSubmit={handleUpload} data={selectedData} />
        </div>
    );
}