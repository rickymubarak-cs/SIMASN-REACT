...
import { DataCardDiklat } from '../components/cards/DataCardDiklat';
...
export default function Diklat({ activeTab, onTabChange }: DiklatProps) {
    ...
    const { data, loading, error, perangkatDaerah, setPerangkatDaerah, refreshData } = useDiklatData();
    const itemsPerPage = 8;

    useEffect(() => {
        console.log('Diklat Page - Data:', data);
        console.log('Diklat Page - Data count:', data.length);
    }, [data]);
    ...
    const filteredData = data.filter(item => {
        ...
        return (
            ...
            (item.nama_usulan_diklat || "").toLowerCase().includes(searchTerm.toLowerCase())
        );
    });
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9]">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                {/* Welcome Section */}
                <div className="bg-gradient-to-r from-violet-600 to-violet-700 rounded-3xl p-6 text-white">
                    <h1 className="text-2xl font-black mb-1">
                        Usulan Diklat
                    </h1>
                    <p className="text-violet-100 text-sm">
                        Kelola dan pantau pengajuan usulan diklat / pengembangan kompetensi pegawai
                    </p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-3xl shadow-sm">
                        ...
                        <p className="text-slate-500 text-sm mt-2 max-w-md mx-auto">
                            Saat ini belum terdapat pengajuan usulan diklat / pengembangan kompetensi
                        ...
                        <button
                            onClick={refreshData}
                            className="mt-6 px-6 py-2 bg-violet-500 text-white rounded-xl text-sm font-bold hover:bg-violet-600 transition-colors"
                        >
                            Refresh Data
                        </button>
                    </div>
