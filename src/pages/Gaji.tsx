...
export default function Gaji({ activeTab, onTabChange }: GajiProps) {
    ...
    const handleTerima = async (id: string, isTembusan: boolean) => {
        try {
            ...
            await gajiService.updateStatus(id, status);
    ...
    const handleEditBerkas = async (id: string, oldFile: string, newFile: File) => {
        ...
        try {
            await gajiService.editBerkas(id, oldFile, newFile);
        ...
        if (paginatedData.length === 0) {
            return (
                <div className="text-center py-20 bg-white rounded-3xl shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Belum Ada Usulan Gaji Berkala</h3>
                    <p className="text-slate-500 text-sm mt-2 max-w-md mx-auto">
                        Saat ini belum terdapat pengajuan usulan Gaji Berkala
                        {unitKerjaName ? ` untuk unit kerja dengan ID ${unitKerjaName}` : ''}.
                    </p>
                    <button
                        onClick={refreshData}
                        className="mt-6 px-6 py-2 bg-teal-500 text-white rounded-xl text-sm font-bold hover:bg-teal-600 transition-colors"
                    >
                        Refresh Data
                    </button>
                </div>
                ...
                <div className="bg-gradient-to-r from-teal-600 to-teal-700 rounded-3xl p-6 text-white">
                    <div className="mb-4">
                        ...
                        <p className="text-teal-100 text-sm">
                            Kelola dan pantau pengajuan usulan Gaji Berkala ASN
                        </p>
                    </div>
