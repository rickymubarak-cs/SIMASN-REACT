...
import { useGajiData } from '../hooks/useGajiData';
import { Navbar } from '../components/layout/Navbar';
import { DashboardSearchBar } from '../components/layout/DashboardSearchBar';
import { DataCardGaji } from '../components/cards/Pelayanan/Admin/Gaji/DataCardGaji';
import { DataCardGajiCompact } from '../components/cards/Pelayanan/Admin/Gaji/DataCardGajiCompact';
import { DataCardGajiDetailed } from '../components/cards/Pelayanan/Admin/Gaji/DataCardGajiDetailed';
import { DataTableGaji } from '../components/tables/DataTableGaji';
import { DetailModalGaji } from '../components/modals/Layanan/Admin/Gaji/DetailModalGaji';
import { ActionModal } from '../components/modals/ActionModal';
import { UploadModalGaji } from '../components/modals/Layanan/Admin/Gaji/UploadModalGaji';
...
export default function Gaji({ activeTab, onTabChange }: GajiProps) {
    ...
    const { data, loading, error, refreshData } = useGajiData();
    ...
    const handlePerbaiki = async (id: string, keterangan: string) => {
        try {
            await gajiService.updateStatus(id, 'perbaikan', keterangan);
            refreshData();
    ...
    const handleTolak = async (id: string, alasan: string) => {
        try {
            await gajiService.updateStatus(id, 'ditolak', alasan);
            refreshData();
    ...
    const handleTerima = async (id: string, isTembusan: boolean) => {
        try {
            await gajiService.updateStatus(id, isTembusan ? 'selesai' : 'diterima');
            refreshData();
    ...
    const handleUpload = async (id: string, file: File) => {
        try {
            await gajiService.uploadBerkas(id, file);
            refreshData();
    ...
    const renderContent = () => {
        ...
        if (paginatedData.length === 0) {
            return <div className="text-center py-20 bg-white rounded-3xl shadow-sm"><div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-8"><DollarSign size={40} className="text-slate-300" /></div><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Belum Ada Usulan Gaji Berkala</h3><p className="text-slate-500 text-sm mt-2">Saat ini belum terdapat pengajuan usulan Gaji Berkala{unitKerjaName ? ` untuk unit kerja ${unitKerjaName}` : ''}.</p><button onClick={refreshData} className="mt-6 px-6 py-2 bg-teal-500 text-white rounded-xl text-sm font-bold hover:bg-teal-600">Refresh Data</button></div>;
        }

        switch (viewMode) {
            case 'compact':
                return <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">{paginatedData.map((item, idx) => <DataCardGajiCompact key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'detailed':
                return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{paginatedData.map((item, idx) => <DataCardGajiDetailed key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'table':
                return <DataTableGaji data={paginatedData} startIndex={(currentPage - 1) * itemsPerPage} onDetail={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />;
            default:
                return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{paginatedData.map((item, idx) => <DataCardGaji key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9]">
            ...
            <DetailModalGaji isOpen={modalState.detail} onClose={() => setModalState(prev => ({ ...prev, detail: false }))} data={selectedData} />
            ...
            <UploadModalGaji isOpen={modalState.upload} onClose={() => setModalState(prev => ({ ...prev, upload: false }))} onSubmit={handleUpload} data={selectedData} />
        </div>
    );
}