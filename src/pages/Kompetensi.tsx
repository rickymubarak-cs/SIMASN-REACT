...
import { DataCardKompetensi } from '../components/cards/Pelayanan/Admin/Kompetensi/DataCardKompetensi';
import { DataCardKompetensiCompact } from '../components/cards/Pelayanan/Admin/Kompetensi/DataCardKompetensiCompact';
import { DataCardKompetensiDetailed } from '../components/cards/Pelayanan/Admin/Kompetensi/DataCardKompetensiDetailed';
import { DataTableKompetensi } from '../components/tables/DataTableKompetensi';
import { DetailModalKompetensi } from '../components/modals/Layanan/Admin/Kompetensi/DetailModalKompetensi';
import { ActionModal } from '../components/modals/ActionModal';
import { UploadModalKompetensi } from '../components/modals/Layanan/Admin/Kompetensi/UploadModalKompetensi';
...
export default function Kompetensi({ activeTab, onTabChange }: KompetensiProps) {
    ...
    const { data, loading, error, refreshData } = useKompetensiData();
    ...
    const filteredData = data.filter(item => {
        ...
        const kompetensi = item.riw_kom_nama || "";
        ...
        return (
            ...
            kompetensi.toLowerCase().includes(searchLower)
        );
    });
    ...
    const handleTerima = async (id: string, isTembusan: boolean) => {
        try {
            await kompetensiService.updateStatus(id, isTembusan ? 'selesai' : 'diterima');
            refreshData();
    ...
    const renderContent = () => {
        ...
        if (paginatedData.length === 0) {
            return <div className="text-center py-20 bg-white rounded-3xl shadow-sm"><div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-8"><Award size={40} className="text-slate-300" /></div><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Belum Ada Usulan Kompetensi</h3><p className="text-slate-500 text-sm mt-2">Saat ini belum terdapat pengajuan usulan Kompetensi / Sertifikasi{unitKerjaName ? ` untuk unit kerja ${unitKerjaName}` : ''}.</p><button onClick={refreshData} className="mt-6 px-6 py-2 bg-orange-500 text-white rounded-xl text-sm font-bold hover:bg-orange-600">Refresh Data</button></div>;
        }

        switch (viewMode) {
            case 'compact':
                return <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">{paginatedData.map((item, idx) => <DataCardKompetensiCompact key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'detailed':
                return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{paginatedData.map((item, idx) => <DataCardKompetensiDetailed key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
            case 'table':
                return <DataTableKompetensi data={paginatedData} startIndex={(currentPage - 1) * itemsPerPage} onDetail={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={(item) => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />;
            default:
                return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{paginatedData.map((item, idx) => <DataCardKompetensi key={idx} data={item} index={idx + 1 + (currentPage - 1) * itemsPerPage} onDetail={() => { setSelectedData(item); setModalState(prev => ({ ...prev, detail: true })); }} onPerbaiki={() => { setSelectedData(item); setModalState(prev => ({ ...prev, perbaiki: true })); }} onTolak={() => { setSelectedData(item); setModalState(prev => ({ ...prev, tolak: true })); }} onTerima={handleTerima} onUpload={() => { setSelectedData(item); setModalState(prev => ({ ...prev, upload: true })); }} />)}</div>;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9]">
            ...
            <DetailModalKompetensi isOpen={modalState.detail} onClose={() => setModalState(prev => ({ ...prev, detail: false }))} data={selectedData} />
            ...
            <UploadModalKompetensi isOpen={modalState.upload} onClose={() => setModalState(prev => ({ ...prev, upload: false }))} onSubmit={handleUpload} data={selectedData} />
        </div>
    );
}